...
function Row({ title, fetchURL, isPoster }) {
    ...
    <div className='row'>
        ...
        <div className='img_container'>
            {movies.map((record) => {
                return <img  key={record.id} className={`img ${(!isPoster) && 'thumbnail'}`} alt={record.name} loading='lazy' onClick={()=> { handleClick(record) }} src={`${baseURL}${isPoster? record.poster_path : record.backdrop_path}`} />
            })}
        </div>
